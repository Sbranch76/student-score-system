...
![E-R图](./assets/fig_er.jpg)
![关系模式图](./assets/mode.jpg)
...
| 姓名 | 班级 | 学号 | git账号 | 承担任务 |
|------|------|------|---------|----------|
| 尹媛 | 24计科二班 | 202405567126 | Sbranch76 | 组长、项目管理、全套文档编写、测试数据构造、测试计划与执行、Git仓库管理 |
| 刘敏祺 | 24计科二班 | 202405567127 | Minqi-Liu | 后端架构搭建、登录鉴权、权限拦截、课表管理模块、文件上传模块、全局异常处理 |
| 朱宁静 | 24计科二班 | 202405567106 | LeonardJane | 学生管理、成绩管理、课程管理、专业管理、教师课程关联模块后端开发、接口联调 |
| 崔慧敏 | 24计科二班 | 202405567131 | cuihuimin2139 | 前端页面开发、路由与权限控制、接口联调、表单校验、界面交互优化 |
| 詹淦栩 | 24计科三班 | 202405567205 | ZXX1217 | 数据库E-R图设计、数据表结构设计、SQL建表脚本编写 |

### 4. Git 提交记录
项目全程采用 Git 进行规范化版本管理，按模块分目录提交，所有代码、文档、脚本、测试记录均完整留痕，累计提交 200+ 条，过程可追溯、可核查。
主要提交内容：
1. 项目初始化、backend/frontend 目录搭建、开发环境配置；
2. sql/ 目录：数据库设计、建表脚本、测试数据构造与提交；
3. backend/ 目录：后端架构、登录认证、权限拦截、业务接口开发；
4. frontend/ 目录：前端页面、路由、接口联调、样式与交互优化；
5. docs/ 目录：需求文档、设计文档、测试文档、项目报告分期提交；
6. 功能迭代、Bug 修复、代码优化、界面完善持续增量更新。
